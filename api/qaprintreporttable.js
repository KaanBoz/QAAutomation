module.exports = function (app, myLocalize, functions, con, router) {
    app.get('/qaprintreporttable', function (req, res) {
        ...
        " and is_deleted = 0 and is_validated = 1 and archived = 0 and added_by =" + sess.user.id;
        if(sess && sess.user){
                    ...
                    con.query("select id, followupid, reportdata, company, report from " + t + whereCondition + " order by " + orderColumnTxt + " " + orderDir + " limit " + limit, function (err, result, fields){
                        ...
                        for (i = 0; i < usersDb.length; i++) { 
                            ...
                            "<a href=\"../qareport?id=" + usersDb[i].id + "&lang=en\">" +
                                ...
                                "</button>" +
                            "</a>" +
                            "<a href=\"../qareportoperation?id=" + usersDb[i].id + "&operation=archive&followupid=" + usersDb[i].followupid + "\">" +
                                "<button type=\"button\" class=\"btn btn-primary btn-xs\" style=\"margin-left:10px;\">" +
                                    "<span class=\"icon-holder\">" +
                                    "<i class=\"c-white-500 ti-archive\"></i>" +
                                    "</span>" +
                                "</button>" +
                            "</a>" +
                            "<a href=\"../qareportoperation?id=" + usersDb[i].id + "&operation=delete&followupid=" + usersDb[i].followupid + "\">" +
                                "<button type=\"button\" class=\"btn btn-danger btn-xs\" style=\"margin-left:10px;\">" +
                                    "<span class=\"icon-holder\">" +
                                    "<i class=\"c-white-500 ti-close\"></i>" +
                                    "</span>" +
                                "</button>" +
                            "</a>" ;
