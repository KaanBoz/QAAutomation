module.exports = function (app, myLocalize, functions, con, router) {
    app.get('/qaedittable', function (req, res) {
        ...
        var t  = "(select id, result, is_deleted, is_validated, followup from analysisresult) as t ";
        var whereCondition = "where (result like '%" + search + "%')" + 
        " and is_deleted = 0 and is_validated = 1 and followup = " + followupId;
        if(sess && sess.user){
            con.query("SELECT COUNT(result) AS analysisresult FROM " + t + " where  is_deleted = 0 and is_validated = 1 and followup = " + followupId , 
            function (err, result, fields){
                if(err) throw err;
                var recordsTotal = result[0].analysisresult;
                con.query("SELECT COUNT(result) AS analysisresult FROM " + t + whereCondition, function (err, result, fields){
                    if(err) throw err;
                    var recordsFiltered = result[0].analysisresult;
                    con.query("select id, result from " + t + whereCondition + " order by " + orderColumnTxt + " " + orderDir + " limit " + limit, function (err, result, fields){
                        ...
                        for (i = 0; i < usersDb.length; i++) { 
                            var buttonView = 
                            "<a href=\"../qaeditresult?id=" + followupId + "&operation=" + "view&resultId=" + usersDb[i].id + "\">" +
                                ...
                                "</button>" +
                            "</a>" +
                            "<a href=\"../qaeditresult?id=" + followupId + "&operation=" + "edit&resultId=" + usersDb[i].id + "\">" +
                                ...
                                "</button>" +
                            "</a>" +
                            "<a href=\"../qaeditresult?id=" + followupId + "&operation=" + "delete&resultId=" + usersDb[i].id + "\">" +
