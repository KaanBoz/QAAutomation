module.exports = function (app, myLocalize, functions, con, router) {
    app.get('/qaedittable', function (req, res) {
        ...
        var t  = "(select id, added_at, is_deleted, is_validated, followup from analysisresult) as t ";
        var whereCondition = "where (added_at like '%" + search + "%') and " + 
        " is_deleted = 0 and is_validated = 1 and followup = " + followupId;
        if(sess && sess.user){
            con.query("SELECT COUNT(id) AS analysisresult FROM " + t + " where  is_deleted = 0 and is_validated = 1 and followup = " + followupId , 
            function (err, result, fields){
                if(err) throw err;
                var recordsTotal = result[0].analysisresult;
                con.query("SELECT COUNT(id) AS analysisresult FROM " + t + whereCondition, function (err, result, fields){
                    if(err) throw err;
