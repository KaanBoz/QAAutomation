module.exports = function (app, myLocalize, functions, con, router, localization) {
                                ...
                                con.query(
                                    ...
                                    if (err){
                                        ...
                                        if(operation == "add"){
                                            ...
                                            con.query(
                                                "select result, detailid from masteralloyresultdetails where is_deleted = 0 and masteralloy = " + resultId, function(err, result, fields){
                                                    var fieldValues = [];
                                                    if(result){
                                                        for(var i = 0; i < values.length; i++){
                                                            var field = {};
    ...
    function removeDetails(headerId){
        con.query("UPDATE masteralloyresultdetails SET is_deleted = 1 where masteralloy=" + headerId, function(err, result, fields){
            if (err){
                message = err.message;
                return;    
            }
